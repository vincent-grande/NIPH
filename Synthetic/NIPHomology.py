...
def total_angle_costs_list(
    ...
    #costs = np.abs(np.log(expected_shifts) - np.log(maxima))**4
    return np.sum(costs)
...
def zero_run_on_points(points):
    parameters = np.array([[i / 12 * np.pi, 1.4] for i in range(12)])
    for scale in np.linspace(1.8, 6.0, 8):
        new_parameters = np.array([[i / 12 * np.pi, scale] for i in range(12)])
...
def extract_PCA_direction(points):
    ...
    return np.arctan(pca.components_[0][0]/pca.components_[0][1])*180/np.pi, explained_variance[0]/explained_variance[1]

def run_best_angle_fit_pca(params):
    # print(params)
    values = params[0]
    percentage = params[1]
    prob_rad = 2
...
def run_best_angle_fit_list(params):
    # print(params)
    if len(params) == 2:
        manual_directions = False
    else:
        manual_directions = True
        probing_directions = params[2]
    values = params[0]
    percentage = params[1]
    prob_rad = 2
    phi = values[0]
    ...
    if manual_directions ==  False:
        probing_directions = np.array([[i / 8 * np.pi, 1.4] for i in range(8)])
        for scale in np.linspace(1.6, 6, 5):
            new_parameters = np.array([[i / 8 * np.pi+(scale+1)/35*np.pi, scale] for i in range(8)])
            probing_directions = np.concatenate((probing_directions, new_parameters), axis=0)
    BCs, BC_list = Compute_Scaled_Deaths_List(
        points, parameters=probing_directions, verbose=False)
    # print(BCs.shape)
    # for BCss in BC_list:
    #    print(BCss.shape)
    maxima = Compute_Maxs(BCs, BC_list, probing_directions, verbose=False)
    return (
        phi,
        best_angle_fit_optimise(maxima, parameters=probing_directions, exponent=4),
        values,
    )
...
def Compute_mult_shifts(BCs, BCxs, dim=1):
    ...
    Gsx = ot.unbalanced.mm_unbalanced(
        weightBCs, weightBCxs, Mx, 1, div="l2", numItermax=1000000
    )
...
def Compute_death_shifts(
    ...
    Gsx = ot.emd(weights_norm, weights_norm_x, Mx, numItermax=1000000)
    shifts = np.exp(
...
def find_max_list(BC_weights, parameters, true_max=True, threshold=False):
    axis_name = "death_shifts"
    maxima = []
    for i, BC_data in enumerate(BC_weights):
        arguments = np.linspace(1.0, parameters[i][1], 1000)
...
def Compute_death_shifts_zero_dim(
    ...
    shifts = np.exp(
        np.divide(Gsx @ log_deathsx[:,0], np.sum(Gsx, axis=1)) - log_deaths[:,0]
    )
    shift_dest = np.exp(
        np.divide(Gsx @ log_deathsx[:,0], np.sum(Gsx, axis=1)))
